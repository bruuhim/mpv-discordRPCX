...
start = int(sys.argv[4]) if len(sys.argv) > 4 and sys.argv[4] != "none" else None
end = int(sys.argv[5]) if len(sys.argv) > 5 and sys.argv[5] != "none" else None
...
try:
	...
	if todo == "shutdown":
		...
		RPC.update(state=state, details=details, start=start, end=end, large_image=large_image, large_text=large_text, small_image=small_image, small_text=small_text)
